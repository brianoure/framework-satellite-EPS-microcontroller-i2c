...
while(True):##MAIN WHILE
    ...
    if (read_PIN_SCL_FROM_OBC()) :# if SCL is high then data is coming in from OBC
        if(    read_PIN1_SDA_FROM_OBC() ) : WORD_FROM_OBC = (WORD_FROM_OBC | (  1<<(55-position56) )) #store 1 bit at position
        if(not read_PIN1_SDA_FROM_OBC() ) : WORD_FROM_OBC = (WORD_FROM_OBC & (~(1<<(55-position56)))) #store 0 bit at position
    ...
    if ( not read_PIN_SCL_FROM_OBC()):#if SCL is low then data is going to OBC
        ...
        set_PIN_SDA_TO_OBC( WORD_TO_OBC & (1<<(55-position56)) )
